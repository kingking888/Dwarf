...
function Dwarf() {
    ...
    this._sendInfos = function (reason, p, ctx, hook) {
        ...
        if (ctx !== null) {
            ...
            data['context'] = ctx;
            const pc = getDwarf().procedure_call_register;
            if (typeof ctx[pc] !== 'undefined') {
                ...
                data['context'] = getDwarf()._extendContextInfo(ctx, hook.showDetails);
            } else {
                // java hook
                data['is_java'] = true;
                data['ptr'] = p;
    ...
    this._extendContextInfo = function (ctx, getSymbols) {
        getSymbols = getSymbols || true;
        const newCtx = {};

        var symb = null;
        if (getSymbols) {
            try {
                symb = DebugSymbol.fromAddress(ctx[getDwarf().procedure_call_register]);
            } catch (e) {
        ...
        for (var reg in ctx) {
            ...
            if (reg === getDwarf().procedure_call_register) {
                if (symb !== null) {
    ...
    this._stalkerCallout = function (context) {
        const ctx = getDwarf()._extendContextInfo(context);
        loggedSend('stalker_callout:::' + JSON.stringify(ctx))
    };
...
function DwarfApi() {
    ...
    this.getTracedTid = function () {
        return getDwarf().native_tracer_tid;
    };
    ...
    this.startNativeTracer = function () {
        if (getDwarf().native_tracer_tid > 0) {
            console.log('Already tracing ' + getDwarf().native_tracer_tid)
        } else {
    ...
    this.stopNativeTracer = function () {
        if (getDwarf().native_tracer_tid === 0) {
            console.log('Not tracing any thread');
        } else {
            Stalker.unfollow(getDwarf().native_tracer_tid);
            getDwarf().native_tracer_tid = 0;
        }
    };
